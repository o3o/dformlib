...
debug (APP_PRINT) {
   ...
   version (DFL_LIB) static assert(0);
}
...
class ApplicationContext {
   ...
   final @property void mainForm(Form mainForm) {
      if (mform) {
   ...
   final @property Form mainForm() nothrow {
      return mform;
   }
...
private extern (Windows) nothrow {
   alias GetTempFileNameWProc = UINT function(LPCWSTR lpPathName,
...
static:
   ...
   void enableVisualStyles() {
      ...
      assert(kernel32);
      {
         ...
         if (createActCtxW) {
            ...
            getTempFileNameW = cast(GetTempFileNameWProc) GetProcAddress(kernel32,
               "GetTempFileNameW");
            ...
            //if(pathbuf)
            {
               ...
               if (pathlen) {
                  ...
                  //if(manifestbuf)
                  {
                     ...
                     if (manifestlen) {
                        HANDLE hf;
                        hf = CreateFileW(manifestbuf.ptr, GENERIC_WRITE, 0,
                           null, CREATE_ALWAYS,
                           FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, HANDLE.init);
                        if (hf != INVALID_HANDLE_VALUE) {
                           DWORD written;
                           if (WriteFile(hf, MANIFEST.ptr, MANIFEST.length, &written,
                                 null)) {
                              CloseHandle(hf);
   ...
   Dstring getSpecialPath(Dstring name) { // package
      ...
      if (ERROR_SUCCESS != RegOpenKeyA(HKEY_CURRENT_USER,
            r"Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders".ptr,
            &hk)) {
      bad_path:
   ...
   @property Dstring userAppDataBasePath() {
      return getSpecialPath("AppData");
   }

   @property bool messageLoop() nothrow {
   ...
   void run(ApplicationContext appcon, void delegate() whileIdle) {
      ...
      version (CUSTOM_MSG_HOOK) {
         HHOOK _msghook = SetWindowsHookExA(WH_CALLWNDPROCRET, &globalMsgHook,
      ...
      try {
         ...
         for (;;) {
            try {
               still_running: while (!(threadFlags & (TF.QUIT | TF.STOP_RUNNING))) {
                  Message msg;

                  //while(PeekMessageA(&msg._winMsg, HWND.init, 0, 0, PM_REMOVE))
                  while (dfl.internal.utf.peekMessage(&msg._winMsg, HWND.init, 0, 0,
                        PM_REMOVE)) {
                     gotMessage(msg);
   ...
   package @property ApplicationContext context() nothrow {
      return ctx;
   }
   ...
   private static class ErrForm : Form {
      ...
      this(Dstring errmsg) {
         ...
         with (label = new Label) {
            ...
            label.text
               = "An application exception has occured. Click Continue to allow " ~ "the application to ignore this error and attempt to continue.";
            parent = this;
         }

         with (errBox = new TextBox) {
            text = errmsg;
            bounds = Rect(PADDING, 40 + PADDING, this.clientSize.width - PADDING * 2,
               50);
   ...
   void addHotkey(Keys k, void delegate(Object sender, KeyEventArgs ea) dg) {
      if (auto pkid = k in hotkeyId) {
         ...
         immutable mod = (k & Keys.MODIFIERS) >> 16, keycode = k & Keys.KEY_CODE;
   ...
   version (DFL_NO_MENUS) {
      ...
      package int addMenuItem(MenuItem menu) {
         ...
         tempmenus = cast(Menu*) realloc(menus, Menu.sizeof * (nmenus + 1));
         if (!tempmenus) {
      ...
      package void addContextMenu(ContextMenu menu) {
         ...
         idx = nmenus;
         nmenus++;
         tempmenus = cast(Menu*) realloc(menus, Menu.sizeof * nmenus);
         if (!tempmenus) {
            nmenus--;
            //throw new OutOfMemory;
            throw new DflException("Out of memory");
      ...
      found:
         if (nmenus == 1) {
            ...
            nmenus--;
            menus = cast(Menu*) realloc(menus, Menu.sizeof * nmenus);
   ...
   @property void autoCollect(bool byes) {
      if (byes) {
         if (!autoCollect) {
            gcinfo = 1;
         }
   ...
   @property bool autoCollect() nothrow {
      return gcinfo > 0;
   }
...
static:
   ...
   version (CUSTOM_MSG_HOOK) DWORD tlsHook; // HHOOK.
   ...
   /+
   @property void filters(IMessageFilter[] filters) {
   ...
   @property IMessageFilter[] filters() nothrow {
      TlsFilterValue* val = cast(TlsFilterValue*)TlsGetValue(tlsFilter);
      if(!val) {
         return null;
      }
   ...
   version (CUSTOM_MSG_HOOK) {
      @property void msghook(HHOOK hhook) {
         TlsSetValue(tlsHook, cast(LPVOID) hhook);
      }

      @property HHOOK msghook() nothrow {
   ...
   @property TF threadFlags() nothrow {
      return cast(TF) cast(DWORD) TlsGetValue(tlsThreadFlags);
   }

   @property void threadFlags(TF flags) {
   ...
   void gotMessage(ref Message msg) {
      ...
      void handleHotkey() {
         ...
         hotkeyHandler[kid](typeid(Application), new KeyEventArgs(cast(Keys)((mod << 16) | keycode)));
      }
...
debug (SHOW_MESSAGE_INFO) {
   ...
   void showMessageInfo(ref Message m) {
      ...
      case WM_SETFOCUS:
         ...
         break;
      case WM_KILLFOCUS:
         writeWm("WM_KILLFOCUS");
         break;
      case WM_ENABLE:
         writeWm("WM_ENABLE");
         break;
      case WM_SETREDRAW:
         writeWm("WM_SETREDRAW");
         break;
      case WM_SETTEXT:
         writeWm("WM_SETTEXT");
         break;
      ...
      case WM_QUERYENDSESSION:
         writeWm("WM_QUERYENDSESSION");
         break;
      case WM_QUIT:
         writeWm("WM_QUIT");
         break;
      case WM_QUERYOPEN:
         writeWm("WM_QUERYOPEN");
         break;
      case WM_ERASEBKGND:
         writeWm("WM_ERASEBKGND");
         break;
      case WM_SYSCOLORCHANGE:
         writeWm("WM_SYSCOLORCHANGE");
         break;
      case WM_ENDSESSION:
         writeWm("WM_ENDSESSION");
         break;
      case WM_SHOWWINDOW:
         writeWm("WM_SHOWWINDOW");
         break;
         //case WM_WININICHANGE: writeWm("WM_WININICHANGE"); break;
      case WM_SETTINGCHANGE:
         writeWm("WM_SETTINGCHANGE");
         break;
      case WM_DEVMODECHANGE:
         writeWm("WM_DEVMODECHANGE");
         break;
      case WM_ACTIVATEAPP:
         writeWm("WM_ACTIVATEAPP");
         break;
      case WM_FONTCHANGE:
         writeWm("WM_FONTCHANGE");
         break;
      case WM_TIMECHANGE:
         writeWm("WM_TIMECHANGE");
         break;
      case WM_CANCELMODE:
         writeWm("WM_CANCELMODE");
         break;
      case WM_SETCURSOR:
         writeWm("WM_SETCURSOR");
         break;
      case WM_MOUSEACTIVATE:
         writeWm("WM_MOUSEACTIVATE");
         break;
      case WM_CHILDACTIVATE:
         writeWm("WM_CHILDACTIVATE");
         break;
      case WM_QUEUESYNC:
         writeWm("WM_QUEUESYNC");
         break;
      case WM_GETMINMAXINFO:
         writeWm("WM_GETMINMAXINFO");
         break;
      case WM_NOTIFY:
         writeWm("WM_NOTIFY");
         break;
      case WM_INPUTLANGCHANGEREQUEST:
         writeWm("WM_INPUTLANGCHANGEREQUEST");
         break;
      case WM_INPUTLANGCHANGE:
         writeWm("WM_INPUTLANGCHANGE");
         break;
      case WM_TCARD:
         writeWm("WM_TCARD");
         break;
      case WM_HELP:
         ...
         break;
      case WM_USERCHANGED:
         writeWm("WM_USERCHANGED");
         break;
      case WM_NOTIFYFORMAT:
         writeWm("WM_NOTIFYFORMAT");
         break;
      case WM_CONTEXTMENU:
         writeWm("WM_CONTEXTMENU");
         break;
      case WM_STYLECHANGING:
         writeWm("WM_STYLECHANGING");
         break;
      case WM_STYLECHANGED:
         writeWm("WM_STYLECHANGED");
         break;
      case WM_DISPLAYCHANGE:
         writeWm("WM_DISPLAYCHANGE");
         break;
      case WM_GETICON:
         writeWm("WM_GETICON");
         break;
      ...
      case WM_NCCREATE:
         ...
         break;
      case WM_NCDESTROY:
         writeWm("WM_NCDESTROY");
         break;
      case WM_NCCALCSIZE:
         writeWm("WM_NCCALCSIZE");
         break;
      case WM_NCHITTEST:
         writeWm("WM_NCHITTEST");
         break;
      case WM_NCPAINT:
         writeWm("WM_NCPAINT");
         break;
      case WM_NCACTIVATE:
         writeWm("WM_NCACTIVATE");
         break;
      case WM_GETDLGCODE:
         writeWm("WM_GETDLGCODE");
         break;
      case WM_NCMOUSEMOVE:
         writeWm("WM_NCMOUSEMOVE");
         break;
      case WM_NCLBUTTONDOWN:
         writeWm("WM_NCLBUTTONDOWN");
         break;
      case WM_NCLBUTTONUP:
         writeWm("WM_NCLBUTTONUP");
         break;
      case WM_NCLBUTTONDBLCLK:
         writeWm("WM_NCLBUTTONDBLCLK");
         break;
      case WM_NCRBUTTONDOWN:
         writeWm("WM_NCRBUTTONDOWN");
         break;
      case WM_NCRBUTTONUP:
         writeWm("WM_NCRBUTTONUP");
         break;
      case WM_NCRBUTTONDBLCLK:
         writeWm("WM_NCRBUTTONDBLCLK");
         break;
      case WM_NCMBUTTONDOWN:
         writeWm("WM_NCMBUTTONDOWN");
         break;
      case WM_NCMBUTTONUP:
         writeWm("WM_NCMBUTTONUP");
         break;
      case WM_NCMBUTTONDBLCLK:
         writeWm("WM_NCMBUTTONDBLCLK");
         break;
      case WM_KEYDOWN:
         writeWm("WM_KEYDOWN");
         break;
      ...
      case WM_SYSCHAR:
         ...
         break;
      case WM_SYSDEADCHAR:
         writeWm("WM_SYSDEADCHAR");
         break;
      case WM_IME_STARTCOMPOSITION:
         writeWm("WM_IME_STARTCOMPOSITION");
         break;
      case WM_IME_ENDCOMPOSITION:
         writeWm("WM_IME_ENDCOMPOSITION");
         break;
      case WM_IME_COMPOSITION:
         writeWm("WM_IME_COMPOSITION");
         break;
      case WM_INITDIALOG:
         writeWm("WM_INITDIALOG");
         break;
      case WM_COMMAND:
         writeWm("WM_COMMAND");
         break;
      case WM_SYSCOMMAND:
         writeWm("WM_SYSCOMMAND");
         break;
      case WM_TIMER:
         writeWm("WM_TIMER");
         break;
      ...
      case WM_INITMENUPOPUP:
         writeWm("WM_INITMENUPOPUP");
         break;
      case WM_MENUSELECT:
         writeWm("WM_MENUSELECT");
         break;
      case WM_MENUCHAR:
         writeWm("WM_MENUCHAR");
         break;
      case WM_ENTERIDLE:
         writeWm("WM_ENTERIDLE");
         break;
      case WM_CTLCOLORMSGBOX:
         writeWm("WM_CTLCOLORMSGBOX");
         break;
      case WM_CTLCOLOREDIT:
         writeWm("WM_CTLCOLOREDIT");
         break;
      case WM_CTLCOLORLISTBOX:
         writeWm("WM_CTLCOLORLISTBOX");
         break;
      case WM_CTLCOLORBTN:
         writeWm("WM_CTLCOLORBTN");
         break;
      case WM_CTLCOLORDLG:
         writeWm("WM_CTLCOLORDLG");
         break;
      case WM_CTLCOLORSCROLLBAR:
         writeWm("WM_CTLCOLORSCROLLBAR");
         break;
      case WM_CTLCOLORSTATIC:
         writeWm("WM_CTLCOLORSTATIC");
         break;
      case WM_MOUSEMOVE:
         writeWm("WM_MOUSEMOVE");
         break;
      case WM_LBUTTONDOWN:
         writeWm("WM_LBUTTONDOWN");
         break;
      case WM_LBUTTONUP:
         writeWm("WM_LBUTTONUP");
         break;
      case WM_LBUTTONDBLCLK:
         writeWm("WM_LBUTTONDBLCLK");
         break;
      case WM_RBUTTONDOWN:
         writeWm("WM_RBUTTONDOWN");
         break;
      case WM_RBUTTONUP:
         writeWm("WM_RBUTTONUP");
         break;
      case WM_RBUTTONDBLCLK:
         writeWm("WM_RBUTTONDBLCLK");
         break;
      case WM_MBUTTONDOWN:
         writeWm("WM_MBUTTONDOWN");
         break;
      case WM_MBUTTONUP:
         writeWm("WM_MBUTTONUP");
         break;
      case WM_MBUTTONDBLCLK:
         writeWm("WM_MBUTTONDBLCLK");
         break;
      case WM_PARENTNOTIFY:
         writeWm("WM_PARENTNOTIFY");
         break;
      case WM_ENTERMENULOOP:
         writeWm("WM_ENTERMENULOOP");
         break;
      case WM_EXITMENULOOP:
         writeWm("WM_EXITMENULOOP");
         break;
      case WM_NEXTMENU:
         writeWm("WM_NEXTMENU");
         break;
      ...
      case WM_USER:
         ...
         break;
      case WM_NEXTDLGCTL:
         writeWm("WM_NEXTDLGCTL");
         break;
      case WM_CAPTURECHANGED:
         writeWm("WM_CAPTURECHANGED");
         break;
      case WM_WINDOWPOSCHANGING:
         writeWm("WM_WINDOWPOSCHANGING");
         break;
      case WM_WINDOWPOSCHANGED:
         writeWm("WM_WINDOWPOSCHANGED");
         break;
      case WM_DRAWITEM:
         writeWm("WM_DRAWITEM");
         break;
      ...
      case WM_MDITILE:
         ...
         break;
      case WM_MDICASCADE:
         writeWm("WM_MDICASCADE");
         break;
      case WM_MDIICONARRANGE:
         writeWm("WM_MDIICONARRANGE");
         break;
      case WM_MDIGETACTIVE:
         writeWm("WM_MDIGETACTIVE");
         break;
      case WM_MOUSEWHEEL:
         writeWm("WM_MOUSEWHEEL");
         break;
      case WM_MOUSEHOVER:
         writeWm("WM_MOUSEHOVER");
         break;
      case WM_MOUSELEAVE:
         writeWm("WM_MOUSELEAVE");
         break;
      case WM_PRINT:
         writeWm("WM_PRINT");
         break;
      case WM_PRINTCLIENT:
         writeWm("WM_PRINTCLIENT");
         break;
      case WM_MEASUREITEM:
         writeWm("WM_MEASUREITEM");
         break;
      ...
      writef("HWND=%d(0x%X) %s WPARAM=%d(0x%X) LPARAM=%d(0x%X)\n\n",
         ...
         ctrl ? ("DFLname='" ~ ctrl.name ~ "'") : "<nonDFL>", m.wParam,
         m.wParam, m.lParam, m.lParam);
...
extern (Windows) {
   ...
   alias IsThemeBackgroundPartiallyTransparentProc = BOOL function(HTHEME hTheme,
...
void _initCommonControls(DWORD dwControls) {
   version (SUPPORTS_COMMON_CONTROLS_EX) {
      ...
      if (!hmodCommonControls) // throw new DflException("Unable to load 'comctl32.dll'");
      {
         goto no_comctl32;
      }

      initProc = cast(InitCommonControlsExProc) GetProcAddress(hmodCommonControls,
...
static this() {
   ...
   version (SUPPORTS_MOUSE_TRACKING) {
      ...
      if (!trackMouseEvent) { // Must be Windows 95; check if common controls has it (IE 5.5).
         trackMouseEvent = cast(TrackMouseEventProc) GetProcAddress(
            GetModuleHandleA("comctl32.dll"), "_TrackMouseEvent");
      }
   }
   ...
   version (SUPPORTS_OPACITY) {
      ...
      setLayeredWindowAttributes = cast(SetLayeredWindowAttributesProc) GetProcAddress(user32,
...
extern (Windows) {
   void _initTextBox() {
      if (!textBoxPrevWndProc) {
         ...
         textBoxPrevWndProc = superClass(HINSTANCE.init, "EDIT", TEXTBOX_CLASSNAME,
            info);
         if (!textBoxPrevWndProc) {
   ...
   void _initListbox() {
      if (!listboxPrevWndProc) {
         dfl.internal.utf.WndClass info;
         listboxPrevWndProc = superClass(HINSTANCE.init, "LISTBOX", LISTBOX_CLASSNAME,
            info);
         if (!listboxPrevWndProc) {
   ...
   void _initButton() {
      if (!buttonPrevWndProc) {
         dfl.internal.utf.WndClass info;
         buttonPrevWndProc = superClass(HINSTANCE.init, "BUTTON", BUTTON_CLASSNAME,
            info);
         if (!buttonPrevWndProc) {
   ...
   void _initMdiclient() {
      if (!mdiclientPrevWndProc) {
         dfl.internal.utf.WndClass info;
         mdiclientPrevWndProc = superClass(HINSTANCE.init, "MDICLIENT", MDICLIENT_CLASSNAME,
            info);
         if (!mdiclientPrevWndProc) {
   ...
   void _initRichtextbox() {
      if (!richtextboxPrevWndProc) {
         ...
         dfl.internal.utf.WndClass info;
         richtextboxPrevWndProc = superClass(HINSTANCE.init, classname,
            RICHTEXTBOX_CLASSNAME, info);
         if (!richtextboxPrevWndProc) {
   ...
   void _initCombobox() {
      if (!comboboxPrevWndProc) {
         dfl.internal.utf.WndClass info;
         comboboxPrevWndProc = superClass(HINSTANCE.init, "COMBOBOX", COMBOBOX_CLASSNAME,
            info);
         if (!comboboxPrevWndProc) {
   ...
   void _initTreeview() {
      if (!treeviewPrevWndProc) {
         ...
         dfl.internal.utf.WndClass info;
         treeviewPrevWndProc = superClass(HINSTANCE.init, "SysTreeView32",
            TREEVIEW_CLASSNAME, info);
         if (!treeviewPrevWndProc) {
   ...
   void _initTabcontrol() {
      if (!tabcontrolPrevWndProc) {
         ...
         dfl.internal.utf.WndClass info;
         tabcontrolPrevWndProc = superClass(HINSTANCE.init, "SysTabControl32",
            TABCONTROL_CLASSNAME, info);
         if (!tabcontrolPrevWndProc) {
   ...
   void _initListview() {
      if (!listviewPrevWndProc) {
         ...
         dfl.internal.utf.WndClass info;
         listviewPrevWndProc = superClass(HINSTANCE.init, "SysListView32",
            LISTVIEW_CLASSNAME, info);
         if (!listviewPrevWndProc) {
   ...
   void _initStatusbar() {
      if (!statusbarPrevWndProc) {
         ...
         dfl.internal.utf.WndClass info;
         statusbarPrevWndProc = superClass(HINSTANCE.init,
            "msctls_statusbar32", STATUSBAR_CLASSNAME, info);
         if (!statusbarPrevWndProc) {
   ...
   void _initProgressbar() {
      if (!progressbarPrevWndProc) {
         ...
         dfl.internal.utf.WndClass info;
         progressbarPrevWndProc = superClass(HINSTANCE.init,
            "msctls_progress32", PROGRESSBAR_CLASSNAME, info);
         if (!progressbarPrevWndProc) {
...
deprecated WNDPROC superClass(HINSTANCE hinst, Dstring className,
   Dstring newClassName, out WNDCLASSA getInfo) { // package
...
// Returns the old wndProc.
WNDPROC superClass(HINSTANCE hinst, Dstring className, Dstring newClassName,
   out dfl.internal.utf.WndClass getInfo) { // package
   WNDPROC wndProc;
   ...
   if (!dfl.internal.utf.registerClass(getInfo)) //throw new DflException("Unable to register window class '" ~ newClassName ~ "'");
   {
      return null;
   }
   return wndProc;
