...
version (DFL_NO_DRAG_DROP) {
   ...
   class DragEventArgs : EventArgs {
      ...
      this(IDataObject dataObj, int keyState, int x, int y,
...
package BOOL enumFirstChildWindows(HWND hwParent, EnumWindowsCallback dg) {
   ...
   efi.cbd.callback = dg;
   scope (exit)
      if (efi.cbd.exception) {
         throw efi.cbd.exception;
      }
...
package:
      ...
      +/
         ...
         final void _simple_front_one(int i) {
            ...
            children = children[0 .. i] ~ children[i + 1 .. i + 2] ~ children[i .. i + 1] ~ children[i
               + 2 .. children.length];
         }
      ...
      final void _simple_back_one(int i) {
         ...
         children = children[0 .. i - 1] ~ children[i + 1 .. i + 2] ~ children[i .. i + 1] ~ children[i
            + 2 .. children.length];
      }
            ...
            if (visible && enabled) {
               ...
               if (hwnd == WindowFromPoint(curpt.point)) {
                  SendMessageA(hwnd, WM_SETCURSOR, cast(WPARAM) hwnd,
                        MAKELPARAM(SendMessageA(hwnd, WM_NCHITTEST, 0,
                              MAKELPARAM(curpt.x, curpt.y)), WM_MOUSEMOVE));
               }
            }
         }
   ...
   private static Font _createCompatibleFont() {
      ...
      try {
         ...
         if (GetVersionExA(&osi) && osi.dwMajorVersion >= 5) {
            ...
            result = new Font("MS Shell Dlg 2",
                  result.getSize(GraphicsUnit.POINT), GraphicsUnit.POINT);
         }
      }
      catch (Throwable) {
      }
   ...
   @property void font(Font f) {
      ...
      wfont = f;
      if (isHandleCreated) {
         SendMessageA(hwnd, WM_SETFONT, cast(WPARAM) wfont.handle, MAKELPARAM(true,
                  0));
      }
      ...
      private static class DropTarget : DflComObject, IDropTarget {
         ...
         HRESULT DragEnter(IDataObject pDataObject, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect) {
            ...
            try {
               ...
               scope DragEventArgs ea = new DragEventArgs(dataObj,
                     cast(int) grfKeyState, pt.x, pt.y,
...
qdoit: scope QueryContinueDragEventArgs ea = new QueryContinueDragEventArgs(
             cast(int) grfKeyState, fEscapePressed != FALSE, act); // ?
       ...
       result = cast(HRESULT) ea.action;
         ...
         HRESULT GiveFeedback(DWORD dwEffect) {
            ...
            try {
               scope GiveFeedbackEventArgs ea = new GiveFeedbackEventArgs(
                     cast(DragDropEffects) dwEffect, true);
   ...
   package final void redrawEntire() {
      if (hwnd) {
         SetWindowPos(hwnd, HWND.init, 0, 0, 0, 0,
   ...
   final void invalidate(bool andChildren) {
      ...
      RedrawWindow(hwnd, null, HRGN.init,
            RDW_ERASE | RDW_INVALIDATE | (andChildren ? RDW_ALLCHILDREN : RDW_NOCHILDREN));
   }
   ...
   final void invalidate(Rect r, bool andChildren) {
      ...
      RedrawWindow(hwnd, &rect, HRGN.init,
            RDW_ERASE | RDW_INVALIDATE | (andChildren ? RDW_ALLCHILDREN : RDW_NOCHILDREN));
   }
   ...
   final void invalidate(Region rgn, bool andChildren) {
      ...
      RedrawWindow(hwnd, null, rgn.handle,
            RDW_ERASE | RDW_INVALIDATE | (andChildren ? RDW_ALLCHILDREN : RDW_NOCHILDREN));
   }
   ...
   final void invoke(void delegate() dg) {
      ...
      if (LRESULT_DFL_INVOKE != SendMessageA(hwnd, wmDfl,
               WPARAM_DFL_INVOKE_SIMPLE, cast(LRESULT)&inv)) {
         throw new DflException("Invoke failure");
      }
      if (inv.exception) {
         throw inv.exception;
   ...
   +/
      ...
      private static bool _eachild(HWND hw, bool delegate(HWND hw) callback,
            ref size_t xiter, bool nested) {
   ...
   package static void eachGoodChildHandle(HWND hwparent,
         bool delegate(HWND hw) callback, bool nested = true) {
   ...
   package static void _dlgselnext(Form dlg, HWND hwcursel, bool forward,
         bool tabStopOnly = true, bool selectableOnly = false, bool nested = true,
         bool wrap = true, HWND hwchildrenof = null) {
      ...
      if (forward) {
         ...
         HWND hwfirst;
         eachGoodChildHandle(hwchildrenof, (HWND hw) {
               assert(!tdone);
               if (hw == hwcursel) {
               foundthis = true;
               } else {
         ...
         if (HWND.init != hwprev) //DefDlgProcA(dlg.handle, WM_NEXTDLGCTL, cast(WPARAM)hwprev, MAKELPARAM(true, 0));
         {
   ...
   package final void _selectNextControl(Form ctrltoplevel, Control ctrl,
      ...
      _dlgselnext(ctrltoplevel, (ctrl
               && ctrl.isHandleCreated) ? ctrl.handle : null, forward, tabStopOnly,
            !tabStopOnly, nested, wrap, this.handle);
   }
   ...
   final void selectNextControl(Control ctrl, bool forward, bool tabStopOnly, bool nested,
         bool wrap) {
      if (!created) {
         return;
      }
      ...
      if (ctrltoplevel) {
         return _selectNextControl(ctrltoplevel, ctrl, forward, tabStopOnly, nested,
               wrap);
      }
   }
            ...
            if (!ctrl.wfont) { // If default.
               if (fon is ctrl.font) { // If same default.
                  if (ctrl.isHandleCreated) {
                     SendMessageA(ctrl.hwnd, WM_SETFONT,
                           cast(WPARAM) fon.handle, MAKELPARAM(true, 0));
                  }
   ...
   protected void onVisibleChanged(EventArgs ea) {
      ...
      if (visible) {
         ...
         if (!hwfocus || (hwfocus == hwnd
                  && !getStyle(ControlStyles.SELECTABLE)) || !IsWindowVisible(hwfocus)) {
      ...
      switch (msg.msg) {
         case WM_PAINT: {
                           ...
                           try {
                              ...
                              scope PaintEventArgs pea = new PaintEventArgs(new Graphics(ps.hdc,
                                       false), Rect(&ps.rcPaint));
         ...
         case WM_ERASEBKGND:
                        if (ctrlStyle & ControlStyles.OPAQUE) {
                           ...
                           scope PaintEventArgs pea = new PaintEventArgs(new Graphics(cast(HDC) msg.wParam,
                                    false), Rect(&uprect));
                           onPaintBackground(pea);
                           msg.result = 1; // Erased.
                        }
         ...
         case WM_PRINTCLIENT:
                        prepareDc(cast(HDC) msg.wParam);
                        scope PaintEventArgs pea = new PaintEventArgs(new Graphics(cast(HDC) msg.wParam,
                                 false), Rect(Point(0, 0), wclientsz));
                        onPaint(pea);
                        return;
         ...
         case WM_WINDOWPOSCHANGING: {
                                       ...
                                       if (!(wp.flags & SWP_NOMOVE) && (location.x != wp.x || location.y != wp.y)) {
                                          scope e = new MovingEventArgs(Point(wp.x, wp.y));
                                          onMoving(e);
                                          wp.x = e.x;
                                          wp.y = e.y;
                                       }
                                       if (!(wp.flags & SWP_NOSIZE) && (width != wp.cx || height != wp.cy)) {
         ...
         case WM_MOUSEMOVE:
                                    ...
                                    if (trackMouseEvent) { // Requires Windows 95 with IE 5.5, 98 or NT4.
                                       if (!menter) {
                                          ...
                                          scope MouseEventArgs mea = new MouseEventArgs(wparamMouseButtons(msg.wParam),
                                                0, pt.x, pt.y, 0);
                                          onMouseEnter(mea);
                                    ...
                                    onMouseMove(new MouseEventArgs(wparamMouseButtons(msg.wParam), 0,
         ...
         case WM_MOUSEWHEEL: { // Requires Windows 98 or NT4.
                                scope MouseEventArgs mea = new MouseEventArgs(
                                      wparamMouseButtons(LOWORD(msg.wParam)), 0,
                                      cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam),
                                      cast(short) HIWORD(msg.wParam));
                                onMouseWheel(mea);
                             }
                             break;
         ...
         case WM_MOUSEHOVER: { // Requires Windows 95 with IE 5.5, 98 or NT4.
                                scope MouseEventArgs mea = new MouseEventArgs(wparamMouseButtons(msg.wParam),
                                      0, cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam), 0);
                                onMouseHover(mea);
                             }
                             break;
         ...
         case WM_LBUTTONDOWN: {
                                 ...
                                 scope MouseEventArgs mea = new MouseEventArgs(MouseButtons.LEFT, 1,
                                       cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam), 0);
                                 onMouseDown(mea);
         ...
         case WM_RBUTTONDOWN: {
                                 scope MouseEventArgs mea = new MouseEventArgs(MouseButtons.RIGHT,
                                       1, cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam), 0);
                                 onMouseDown(mea);
                              }
                              break;

         case WM_MBUTTONDOWN: {
                                 scope MouseEventArgs mea = new MouseEventArgs(MouseButtons.MIDDLE,
                                       1, cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam), 0);
                                 onMouseDown(mea);
                              }
                              break;
         ...
         case WM_LBUTTONDBLCLK: {
                                   scope MouseEventArgs mea = new MouseEventArgs(MouseButtons.LEFT, 2,
                                         cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam), 0);
                                   onMouseDown(mea);

                                   if (
                                         (ctrlStyle & (ControlStyles.STANDARD_CLICK | ControlStyles.STANDARD_DOUBLE_CLICK)) == (
         ...
         case WM_RBUTTONDBLCLK: {
                                   scope MouseEventArgs mea = new MouseEventArgs(MouseButtons.RIGHT,
                                         2, cast(short) LOWORD(msg.lParam), cast(short) HIWORD(msg.lParam), 0);
                                   onMouseDown(mea);
                                }
                                break;
         ...
         case WM_CREATE:
                                                  try {
                                                     ...
                                                     if (recreatingHandle) {
                                                        ...
                                                        delayInvoke(function(Control cthis, size_t[] params) {
                                                              cthis.cbits &= ~CBits.RECREATING;
                                                              });
                                                     }
                                                  }
                                                  catch (DThrowable e) {
   ...
   package final void doShow() {
      if (wparent) { // Exclude owner.
         SetWindowPos(hwnd, HWND.init, 0, 0, 0, 0,
               SWP_NOSIZE | SWP_NOMOVE | SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOZORDER);
      } else {
...
create_err:
      ...
      hwnd = dfl.internal.utf.createWindowEx(wexstyle, className, wtext,
            wstyle, wrect.x, wrect.y, wrect.width, wrect.height, wparent.handle,
            HMENU.init, Application.getInstance(), null);
      if (!hwnd) {
         goto create_err;
      }
   }
...
create_err:
      ...
      with (cp) {
         ...
         Application.creatingControl(this);
         hwnd = dfl.internal.utf.createWindowEx(exStyle, className, caption,
               (style & ~WS_VISIBLE), x, y, width, height, parent, menu, inst, param);
         if (!hwnd) {
            debug (APP_PRINT) {
               cprintf(
                     "CreateWindowEx failed." ~ " (exStyle=0x%X, className=`%.*s`, caption=`%.*s`, style=0x%X, x=%d, y=%d, width=%d, height=%d," ~ " parent=0x%X, menu=0x%X, inst=0x%X, param=0x%X)\n",
                     exStyle, className.ptr, caption.ptr, style, x, y, width,
                     height, parent, menu, inst, param);
            }
      ...
      version (RADIO_GROUP_LAYOUT) {
         ...
         foreach (Control ctrl; ccollection) {
            ...
            if (ctrl._rtype() & 1) { // Radio type.
               ...
               wlg = ctrl._style();
               if (foundRadio) {
                  if (wlg & WS_GROUP) //ctrl._style(wlg & ~WS_GROUP);
                  {
   ...
   protected void onLayout(LayoutEventArgs lea) {
      ...
      foreach (Control ctrl; ccollection) {
         ...
         switch (ctrl.sdock) {
            ...
            case DockStyle.LEFT:
               ctrl.setBoundsCore(area.x, area.y, 0,
                     area.height, cast(BoundsSpecified)(BoundsSpecified.LOCATION | BoundsSpecified.HEIGHT));
               area.x = area.x + ctrl.width;
               area.width = area.width - ctrl.width;
               break;

            case DockStyle.TOP:
               ctrl.setBoundsCore(area.x, area.y, area.width, 0,
                     cast(BoundsSpecified)(BoundsSpecified.LOCATION | BoundsSpecified.WIDTH));
            ...
            case DockStyle.BOTTOM:
               ctrl.setBoundsCore(area.x,
                     area.bottom - ctrl.height, area.width, 0,
                     cast(BoundsSpecified)(BoundsSpecified.LOCATION | BoundsSpecified.WIDTH));
               area.height = area.height - ctrl.height;
               break;

            case DockStyle.RIGHT:
               ctrl.setBoundsCore(area.right - ctrl.width,
                     area.y, 0, area.height,
                     cast(BoundsSpecified)(BoundsSpecified.LOCATION | BoundsSpecified.HEIGHT));
               area.width = area.width - ctrl.width;
               break;
   ...
   protected bool processKeyEventArgs(ref Message msg) {
      switch (msg.msg) {
         ...
         case WM_CHAR: {
                          scope KeyPressEventArgs kpea = new KeyPressEventArgs(cast(dchar) msg.wParam,
                                modifierKeys);
                          onKeyPress(kpea);
                          if (kpea.handled) {
                             return true;
...
final:
   ...
   @property void killing(bool byes) //{ if(byes) cbits |= CBits.KILLING; else cbits &= ~CBits.KILLING; }
{
   assert(byes);
   if (byes) {
      cbits |= CBits.KILLING;
...
package abstract class ControlSuperClass : Control {
   ...
   protected override void wndProc(ref Message msg) {
      switch (msg.msg) {
         ...
         case WM_PRINTCLIENT:
            ...
            scope Graphics gpx = new CommonGraphics(hwnd, GetDC(hwnd));
            scope PaintEventArgs pea = new PaintEventArgs(gpx, Rect(Point(0, 0),
                     wclientsz));
...
class ScrollableControl : Control {
   ...
   body {
      area.width = cast(int)(
            cast(float) area.width / cast(float) fromScale.width * cast(float) toScale.width);
      area.height = cast(int)(
            cast(float) area.height / cast(float) fromScale.height * cast(float) toScale.height);
      return area;
   }
   ...
   override protected void onControlAdded(ControlEventArgs ea) {
   ...
   override @property Rect displayRectangle() {
      ...
      result.location = Point(result.location.x - scrollPosition.x,
            result.location.y - scrollPosition.y);
...
class ContainerControl : ScrollableControl, IContainerControl {
...
shared class SharedControl {
   private:
      ...
      LPARAM makeParam(ARGS...)(void function(Control, ARGS) fn, Tuple!(ARGS)* args) if (ARGS.length) {
         static assert((DflInvokeParam*).sizeof <= LPARAM.sizeof);
         ...
         // FIX: alias malloc = dfl.internal.clib.malloc;
         // FIX: alias free = dfl.internal.clib.free;
   ...
   public:
      ...
      void invoke(ARGS...)(void function(Control, ARGS) fn, ARGS args) if (ARGS.length
      ...
      void delayInvoke(ARGS...)(void function(Control, ARGS) fn, ARGS args) if (
            ARGS.length && !hasLocalAliasing!(ARGS)) {
         auto ctrl = cast(Control) _ctrl;
         auto hwnd = ctrl.handle;
      ...
      void delayInvoke(ARGS...)(void function(Control, ARGS) fn, ARGS args) if (!ARGS.length) {
         auto ctrl = cast(Control) _ctrl;
         auto hwnd = ctrl.handle;
