...
template Event(T1, T2) {
   ...
   struct Event {
      ...
      @property bool hasHandlers() pure nothrow {
         return _array.length > 1;
      }

      // Use opApply and hasHandlers instead.
      deprecated @property Handler[] handlers() pure nothrow {
         if (!hasHandlers) {
            return null;
         }
         try {
...
class EventArgs {
   ...
   static @property EventArgs empty() nothrow {
      return _e;
   }
}
...
class ThreadExceptionEventArgs : EventArgs {
   ...
   final @property DThrowable exception() pure nothrow {
      return except;
   }
