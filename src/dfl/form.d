...
enum FormBorderStyle : ubyte { //: BorderStyle
   ...
   FIXED_3D = BorderStyle.FIXED_3D,
   FIXED_SINGLE = BorderStyle.FIXED_SINGLE,
   FIXED_DIALOG,
   SIZABLE,
   FIXED_TOOLWINDOW,
   SIZABLE_TOOLWINDOW,
...
class Form : ContainerControl, IDialogResult {
   ...
   final @property IButtonControl acceptButton() {
      return acceptBtn;
   }

   final @property void cancelButton(IButtonControl btn) {
      cancelBtn = btn;
   ...
   final @property IButtonControl cancelButton() {
      return cancelBtn;
   }

   // An exception is thrown if the shortcut was already added.
   final void addShortcut(Keys shortcut, void delegate(Object sender,
            FormShortcutEventArgs ea) pressed)
      in {
   ...
   final void addShortcut(Keys shortcut, void delegate(Object sender, EventArgs ea) pressed) {
      return addShortcut(shortcut, cast(void delegate(Object sender,
                  FormShortcutEventArgs ea)) pressed);
   }
   ...
   final @property Form getActiveMdiChild() {
      return cast(Form) fromHandle(cast(HWND) SendMessageA(handle, WM_MDIGETACTIVE,
               0, 0));
   }
   ...
   final @property Point screenCenter() {
      Rect area;
   ...
   final @property Point parentCenter() {
      Control cwparent;
      if (wstyle & WS_CHILD) {
         cwparent = wparent;
      } else {
...
create_err:
      ...
      if (wowner) // wowner.createHandle(); // DMD 0.111: class dfl.control.Control member createHandle is not accessible
      {
...
show_normal:
   ...
   /+
      ...
      override @property void autoScroll(bool byes) {
         super.autoScroll(byes);
      }


   override @property bool autoScroll() {
   ...
   +/
      ...
      final @property void controlBox(bool byes) {
         if (byes) {
   ...
   final @property bool controlBox() {
      return (_style() & WS_SYSMENU) != 0;
   }

   final @property void desktopBounds(Rect r) {
      RECT rect;
      if (r.width < 0) {
         r.width = 0;
      }
      ...
      setBoundsCore(rect.left, rect.top, rect.right - rect.left,
            rect.bottom - rect.top, BoundsSpecified.ALL);
   }

   final @property Rect desktopBounds() {
      RECT r;
      GetWindowRect(handle, &r);
      return Rect(&r);
   }

   final @property void desktopLocation(Point dp) {
      //Control par = parent;
      //if(par) // Convert from screen coords to parent coords.
   ...
   final @property Point desktopLocation() {
      RECT r;
      GetWindowRect(handle, &r);
      return Point(r.left, r.top);
   }

   final @property void dialogResult(DialogResult dr) {
      fresult = dr;
   ...
   final @property DialogResult dialogResult() {
      return fresult;
   }
   ...
   final @property void formBorderStyle(FormBorderStyle bstyle) {
      ...
      enum DWORD EXSTNOTNONE = ~(
            WS_EX_TOOLWINDOW | WS_EX_CLIENTEDGE | WS_EX_DLGMODALFRAME | WS_EX_STATICEDGE | WS_EX_WINDOWEDGE);
      ...
      if (isHandleCreated) {
         if (vis) {
            ...
            SetWindowPos(hwnd, HWND.init, 0, 0, 0, 0,
                  SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER); // Recalculate the frame while hidden.
            _resetSystemMenu();
            // Do it directly so that DFL code can't prevent it.
            doShow();
            invalidate(true);
            ...
            SetWindowPos(hwnd, HWND.init, 0, 0, 0, 0,
                  SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE); // Recalculate the frame.
            _resetSystemMenu();
         }
      }
   }

   final @property FormBorderStyle formBorderStyle() {
      LONG st = _style();
      LONG exst = _exStyle();
   ...
   final @property void helpButton(bool byes) {
      if (byes) {
   ...
   final @property bool helpButton() {
   ...
   final @property void icon(Icon ico) {
      wicon = ico;
   ...
   final @property bool isMdiChild() {
      return (_exStyle() & WS_EX_MDICHILD) != 0;
   }

   version (NO_MDI) {
      private alias MdiClient = Control; // ?
   }
   ...
   version (NO_MDI) {
      ...
      final @property void isMdiContainer(bool byes) {
         if (mdiClient) {
            if (!byes) {
               // Remove MDI client.
               mdiClient.dispose();
      ...
      final @property bool isMdiContainer() {
         version (NO_MDI) {
            return false;
         } else {
      ...
      version (NO_MDI) {
         ...
         final @property void mdiParent(Form frm)
            in {
               if (frm) {
   ...
   final @property void maximizeBox(bool byes) {
      if (byes == maximizeBox) {
         return;
      }
   ...
   final @property bool maximizeBox() {
      return (_style() & WS_MAXIMIZEBOX) != 0;
   }

   final @property void minimizeBox(bool byes) {
      if (byes == minimizeBox) {
         return;
      }
   ...
   final @property bool minimizeBox() {
   ...
   private bool _isPaintingSizeGrip() {
      if (grip) {
         if (wstyle & WS_THICKFRAME) {
            return !(wstyle & (WS_MINIMIZE | WS_MAXIMIZE | WS_VSCROLL | WS_HSCROLL));
         }
      }
      return false;
   }
   ...
   version (DFL_NO_MENUS) {
      ...
      final @property void menu(MainMenu menu) {
         if (isHandleCreated) {
            HWND hwnd;
            hwnd = handle;
      ...
      final @property MainMenu menu() {
         return wmenu;
      }

      /+

         final @property MainMenu mergedMenu() {
            // Return menu belonging to active MDI child if maximized ?
         }
      +/
   }

   final @property void minimumSize(Size min) {
   ...
   final @property Size minimumSize() {
      return minsz;
   }

   final @property void maximumSize(Size max) {
   ...
   final @property Size maximumSize() {
      return maxsz;
   }

   final @property bool modal() {
      return wmodal;
   }

   // If opacity and transparency are supported.
   static @property bool supportsOpacity() {
   ...
   final @property void opacity(double opa) {
      if (setLayeredWindowAttributes) {
         BYTE alpha;
         ...
         if (alpha == BYTE.max) { // Disable
            ...
            if (isHandleCreated) {
               ...
               if (transKey == Color.empty) {
                  ...
                  setLayeredWindowAttributes(handle, transKey.toRgb(), alpha,
   ...
   /+
      ...
      final @property Form[] ownedForms() {
         // TODO: implement.
      }
   +/
   ...
   final @property Form owner() {
      return wowner;
   }

   // This function does not work in all cases.
   final @property void showInTaskbar(bool byes) {
      if (isHandleCreated) {
         bool vis;
         vis = visible;
   ...
   final @property bool showInTaskbar() {
   ...
   final @property void startPosition(FormStartPosition startpos) {
      this.startpos = startpos;
   }

   final @property FormStartPosition startPosition() {
      return startpos;
   }

   final @property void topMost(bool byes) {
      /+
         if(byes) {
      ...
      +/
         ...
         if (isHandleCreated) {
            SetWindowPos(handle, byes ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0, 0,
   ...
   final @property bool topMost() {
      return (_exStyle() & WS_EX_TOPMOST) != 0;
   }

   final @property void transparencyKey(Color c) {
      if (setLayeredWindowAttributes) {
         transKey = c;
         BYTE alpha = opacityToAlpha(opa);
   ...
   final @property Color transparencyKey() {
      return transKey;
   }

   final @property void windowState(FormWindowState state) {
   ...
   final @property FormWindowState windowState() {
      LONG wl;
   ...
   final void activate() {
      ...
      version (NO_MDI) {
         ...
         if (isMdiChild) {
            ...
            SendMessageA(mdiParent.mdiClient.handle, WM_MDIACTIVATE, cast(WPARAM) handle,
                  0);
            return;
         }
      }
...
bad_owner:
   ...
   package final void _selectChild(Control ctrl) {
      if (ctrl.canSelect) {
         ...
         DefDlgProcA(this.hwnd, WM_NEXTDLGCTL, cast(WPARAM) ctrl.hwnd, MAKELPARAM(true,
                  0));
               ...
               +/
                  ...
                  version (NO_MDI) {
                     ...
                     if (isMdiChild) {
                        ...
                        msg.result = dfl.internal.utf.defMDIChildProc(msg.hWnd, msg.msg,
...
def_def:
               version (NO_MDI) {
                  ...
                  if (mdiClient && mdiClient.isHandleCreated && msg.msg != WM_SIZE) //msg.result = DefFrameProcA(msg.hWnd, mdiClient.handle, msg.msg, msg.wParam, msg.lParam);
                  {
                     msg.result = dfl.internal.utf.defFrameProc(msg.hWnd,
                           mdiClient.handle, msg.msg, msg.wParam, msg.lParam);
                  } else if (isMdiChild) //msg.result = DefMDIChildProcA(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
                  {
                     msg.result = dfl.internal.utf.defMDIChildProc(msg.hWnd, msg.msg,
                           msg.wParam, msg.lParam);
                  } else //msg.result = DefDlgProcA(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
                  {
                     msg.result = _defFormProc(msg.hWnd, msg.msg, msg.wParam, msg.lParam);
                  }
               }
   ...
   version (NO_MDI) {
      ...
      protected final @property MdiClient mdiClient() {
         return _mdiClient;
      }
   }
      ...
      protected bool preFilterMessage(ref Message m) {
         version (NO_MDI)
            ...
            bool mdistuff = form.mdiClient && form.mdiClient.isHandleCreated
               && (form.mdiClient.handle == m.hWnd || IsChild(form.mdiClient.handle,
                        m.hWnd));
         ...
         if (mdistuff) {
            ...
            switch (m.msg) {
                              ...
                              if (WM_KEYDOWN == m.msg) {
                                 switch (cast(Keys) m.wParam) {
                                    case Keys.UP, Keys.LEFT:
                                       // Backwards...
                                       Control._dlgselnext(form,
                                             m.hWnd, false, false, true);
                                       break;
                                    case Keys.DOWN, Keys.RIGHT:
                                       // Forwards...
                                       Control._dlgselnext(form,
                                             m.hWnd, true, false, true);
                                       break;
                                    default:
                                       assert(0);
                                 }
               ...
               case WM_SYSCHAR: {
                                   ...
                                   bool foundmhw = false;
                                   bool foundmn = false;
                                   eachGoodChildHandle(form.handle, (HWND hw) {
                                         if (foundmhw) {
                                         if (pmnemonic(hw)) {
                                         foundmn = true;
                                         return false; // Break.
   ...
   +/
      ...
      final void _recalcClientSize() {
         ...
         wclientsz = Size(wrect.width - ((r.right - r.left) - wrect.width),
               wrect.height - ((r.bottom - r.top) - wrect.height));
      }
}
...
create_err:
      ...
      +/
         ...
         protected override void prevWndProc(ref Message msg) {
            ...
            msg.result = dfl.internal.utf.callWindowProc(mdiclientPrevWndProc,
...
init_err:
      ...
      _hwPark = CreateWindowExA(WS_EX_TOOLWINDOW, PARK_CLASSNAME.ptr, "",
            WS_OVERLAPPEDWINDOW, 0, 0, 0, 0, HWND.init, HMENU.init, wce.hInstance, null);
      if (!_hwPark) {
         debug (APP_PRINT)
