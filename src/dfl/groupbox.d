...
class GroupBox : ControlSuperClass {
   override @property Rect displayRectangle() {
      ...
      return Rect(xw, yw + _textHeight, clientSize.width - xw * 2,
         clientSize.height - yw - _textHeight - yw);
   }
   ...
   version (DFL_NO_DRAG_DROP) {
      ...
      override @property void allowDrop(bool dyes) {
         //if(dyes)
      ...
      if (WM_PAINT == msg.msg) {
         ...
         if (hmuxt) {
            ...
            if (isAppThemed && isAppThemed()) {
               auto txt = text;
               if (txt.length) {
                  auto openThemeData = cast(typeof(&OpenThemeData)) GetProcAddress(hmuxt,
                     "OpenThemeData");
                  HTHEME htd;
                  if (openThemeData && HTHEME.init != (htd = openThemeData(msg.hWnd,
                        "Button"))) {
                     ...
                     try {
                        ...
                        Color c;
                        COLORREF cr;
                        auto getThemeColor = cast(typeof(&GetThemeColor)) GetProcAddress(hmuxt,
                           "GetThemeColor");
                        auto gtcState = enabled ? (1 /*PBS_NORMAL*/ ) : (2 /*GBS_DISABLED*/ );
                        if (getThemeColor && 0 == getThemeColor(htd, 4 /*BP_GROUPBOX*/ ,
                              gtcState, 3803 /*TMT_TEXTCOLOR*/ , &cr)) {
                           c = Color.fromRgb(cr);
                        } else {
                        ...
                        g.drawText(txt, f, c, Rect(8 + 2, 0, tsz.width, tsz.height),
                           tfmt);
                     }
                     finally {
                        //if(ps.hdc)
                        ...
                        auto closeThemeData = cast(typeof(&CloseThemeData)) GetProcAddress(hmuxt,
                           "CloseThemeData");
