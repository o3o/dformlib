...
version (DFL_UNICODE) version = STATIC_UNICODE;
...
private Dstring _toAnsiz(Dstring utf8, bool safe = true) {
   ...
   if (safe) //return stringToStringz(utf8)[0 .. utf8.length];
   {
...
extern (Windows) {
   alias CreateWindowExWProc = HWND function(DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName,
   ...
   alias CallWindowProcWProc = LRESULT function(WNDPROC lpPrevWndFunc, HWND hWnd,
         UINT Msg, WPARAM wParam, LPARAM lParam);
   alias RegisterClipboardFormatWProc = UINT function(LPCWSTR lpszFormat);
   alias GetClipboardFormatNameWProc = int function(UINT format,
         LPWSTR lpszFormatName, int cchMaxCount);
   alias DrawTextExWProc = int function(HDC hdc, LPWSTR lpchText, int cchText,
         LPRECT lprc, UINT dwDTFormat, LPDRAWTEXTPARAMS lpDTParams);
   ...
   alias RegSetValueExWProc = LONG function(HKEY hKey, LPCWSTR lpValueName,
         DWORD Reserved, DWORD dwType, BYTE* lpData, DWORD cbData);
   alias RegCreateKeyExWProc = LONG function(HKEY hKey, LPCWSTR lpSubKey,
   ...
   alias RegOpenKeyExWProc = LONG function(HKEY hKey, LPCWSTR lpSubKey,
   ...
   alias RegQueryValueExWProc = LONG function(HKEY hKey, LPCWSTR lpValueName,
   ...
   alias GetTextExtentPoint32WProc = BOOL function(HDC hdc, LPCWSTR lpString,
         int cbString, LPSIZE lpSize);
   alias LoadImageWProc = HANDLE function(HINSTANCE hinst, LPCWSTR lpszName,
   ...
   alias PeekMessageWProc = BOOL function(LPMSG lpMsg, HWND hWnd,
   ...
   alias DefFrameProcWProc = LRESULT function(HWND hWnd, HWND hWndMDIClient,
         UINT uMsg, WPARAM wParam, LPARAM lParam);
   alias DefMDIChildProcWProc = LRESULT function(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
   alias GetClassInfoWProc = BOOL function(HINSTANCE hInstance,
         LPCWSTR lpClassName, LPWNDCLASSW lpWndClass);
   alias FindFirstChangeNotificationWProc = HANDLE function(LPCWSTR lpPathName,
         BOOL bWatchSubtree, DWORD dwNotifyFilter);
   alias GetFullPathNameWProc = DWORD function(LPCWSTR lpFileName,
...
HWND createWindowEx(DWORD dwExStyle, Dstring className, Dstring windowName, DWORD dwStyle,
   ...
   if (useUnicode) {
      ...
      return proc(dwExStyle, toUnicodez(className), toUnicodez(windowName),
            dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
   } else {
      return CreateWindowExA(dwExStyle, unsafeAnsiz(className), unsafeAnsiz(windowName),
            dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
   }
}
...
HWND createWindow(Dstring className, Dstring windowName, DWORD dwStyle, int x, int y,
   ...
   return createWindowEx(0, className, windowName, dwStyle, x, y, nWidth,
...
Dstring dragQueryFile(HDROP hDrop, UINT iFile) {
   ...
   if (useUnicode) {
      version (STATIC_UNICODE) {
         ...
         if (!proc) {
            proc = cast(DragQueryFileWProc) GetProcAddress(GetModuleHandleA("shell32.dll"),
                  NAME.ptr);
            if (!proc) {
               getProcErr(NAME);
            }
...
HANDLE createFile(Dstring fileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes,
   ...
   if (useUnicode) {
      return CreateFileW(toUnicodez(fileName), dwDesiredAccess, dwShareMode,
            lpSecurityAttributes, dwCreationDistribution, dwFlagsAndAttributes, hTemplateFile);
   } else {
      return CreateFileA(unsafeAnsiz(fileName), dwDesiredAccess, dwShareMode,
            lpSecurityAttributes, dwCreationDistribution, dwFlagsAndAttributes, hTemplateFile);
   }
}
...
BOOL peekMessage(MSG* pmsg, HWND hwnd = HWND.init, UINT wmFilterMin = 0,
      UINT wmFilterMax = 0, UINT removeMsg = PM_NOREMOVE) {
   if (useUnicode) {
      version (STATIC_UNICODE_PEEK_MESSAGE) {
...
Dstring regQueryValueString(HKEY hkey, Dstring valueName, LPDWORD lpType = null) {
   ...
   if (useUnicode) {
      ...
      char[] s = new char[sz];
      if (ERROR_SUCCESS != RegQueryValueExA(hkey, lpValueName, null, null,
            cast(LPBYTE) s.ptr, &sz)) {
         return null;
      }
