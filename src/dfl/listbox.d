...
abstract class ListControl : ControlSuperClass {
   ...
   abstract @property void selectedIndex(int idx);

   abstract @property int selectedIndex();

   abstract @property void selectedValue(Object val);

   abstract @property void selectedValue(Dstring str);

   abstract @property Object selectedValue();
...
class ListBox : ListControl {
   ...
   static class SelectedIndexCollection {
      ...
      int opApply(int delegate(ref int) dg) {
         ...
         if (lbox.isMultSel()) {
            ...
            if (items.length != lbox.prevwproc(LB_GETSELITEMS, items.length,
                  cast(LPARAM) cast(int*) items)) {
   ...
   @property void drawMode(DrawMode dm) {
   ...
   final @property void horizontalExtent(int he) {
      if (isHandleCreated) {
   ...
   final @property int horizontalExtent() {
      if (isHandleCreated) {
   ...
   final @property void horizontalScrollbar(bool byes) {
      if (byes) {
   ...
   final @property bool horizontalScrollbar() {
      return (_style() & WS_HSCROLL) != 0;
   }

   final @property void integralHeight(bool byes) {
      if (byes) {
   ...
   final @property ObjectCollection items() {
      return icollection;
   }

   final @property void multiColumn(bool byes) {
   ...
   final @property bool multiColumn() {
      return (_style() & LBS_MULTICOLUMN) != 0;
   }

   final @property void scrollAlwaysVisible(bool byes) {
      if (byes) {
   ...
   final @property bool scrollAlwaysVisible() {
      return (_style() & LBS_DISABLENOSCROLL) != 0;
   }

   override @property void selectedIndex(int idx) {
      if (isHandleCreated) {
         if (isMultSel()) {
            if (idx == -1) {
               ...
               if (items.length != prevwproc(LB_GETSELITEMS, items.length,
                     cast(LPARAM) cast(int*) items)) {
   ...
   override @property int selectedIndex() {
      if (isHandleCreated) {
         if (isMultSel()) {
   ...
   final @property void selectedItem(Object o) {
      int i;
      i = items.indexOf(o);
      if (i != -1) {
         selectedIndex = i;
   ...
   final @property void selectedItem(Dstring str) {
      int i;
      i = items.indexOf(str);
      if (i != -1) {
         selectedIndex = i;
   ...
   final @property Object selectedItem() {
      int idx;
      idx = selectedIndex;
      if (idx == -1) {
         return null;
   ...
   override @property void selectedValue(Object val) {
      selectedItem = val;
   }

   override @property void selectedValue(Dstring str) {
      selectedItem = str;
   }

   override @property Object selectedValue() {
      return selectedItem;
   }
   ...
   final @property SelectedObjectCollection selectedItems() {
      return selobjcollection;
   }

   @property void selectionMode(SelectionMode selmode) {
   ...
   final @property void sorted(bool byes) {
      /+
      if(byes) {
   ...
   final @property bool sorted() {
      //return (_style() & LBS_SORT) != 0;
      return _sorting;
   }

   final @property void topIndex(int idx) {
      if (isHandleCreated) {
         prevwproc(LB_SETTOPINDEX, idx, 0);
      }
   }

   final @property int topIndex() {
      if (isHandleCreated) {
   ...
   final @property void useTabStops(bool byes) {
      if (byes) {
   ...
   final @property bool useTabStops() {
   ...
   final int findString(Dstring str, int startIndex) {
      ...
      if (created) {
         if (dfl.internal.utf.useUnicode) {
            result = prevwproc(LB_FINDSTRING, startIndex,
               cast(LPARAM) dfl.internal.utf.toUnicodez(str));
         } else {
            result = prevwproc(LB_FINDSTRING, startIndex,
               cast(LPARAM) dfl.internal.utf.unsafeAnsiz(str));
         }
   ...
   final int findStringExact(Dstring str, int startIndex) {
      ...
      if (created) {
         if (dfl.internal.utf.useUnicode) {
            result = prevwproc(LB_FINDSTRINGEXACT, startIndex,
               cast(LPARAM) dfl.internal.utf.toUnicodez(str));
         } else {
            result = prevwproc(LB_FINDSTRINGEXACT, startIndex,
               cast(LPARAM) dfl.internal.utf.unsafeAnsiz(str));
   ...
   private:
      ...
      void _added(size_t idx, Object val) {
         if (lbox.created) {
            if (dfl.internal.utf.useUnicode) {
               lbox.prevwproc(LB_INSERTSTRING, idx,
                  cast(LPARAM) dfl.internal.utf.toUnicodez(getObjectString(val)));
            } else {
               lbox.prevwproc(LB_INSERTSTRING, idx,
                  cast(LPARAM) dfl.internal.utf.toAnsiz(getObjectString(val))); // Can this be unsafeAnsiz()?
            }
         }
      }
   ...
   public:
      ...
      mixin ListWrapArray!(Object, _items, _blankListCallback!(Object),
         _added, _blankListCallback!(Object), _removed, true, false, false) _wraparray;
   }
...
protected:
   ...
   override void prevWndProc(ref Message msg) {
      ...
      msg.result = dfl.internal.utf.callWindowProc(listboxPrevWndProc, msg.hWnd,
   ...
   override void wndProc(ref Message msg) {
      ...
      case LB_INSERTSTRING:
         ...
         msg.result = icollection.insert2(msg.wParam,
            cast(Dstring) stringFromStringz(cast(char*) msg.lParam).dup); // TODO: fix. // Needed in D2.
         return;
...
final:
   LRESULT prevwproc(UINT msg, WPARAM wparam, LPARAM lparam) {
      ...
      return dfl.internal.utf.callWindowProc(listboxPrevWndProc, hwnd, msg, wparam,
         lparam);
   }
}
