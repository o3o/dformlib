...
class NotifyIcon {
   version (DFL_NO_MENUS) {
      ...
      final @property void contextMenu(ContextMenu menu) {
         this.cmenu = menu;
      }

      final @property ContextMenu contextMenu() {
         return cmenu;
      }
   }
   ...
   final @property void text(Dstring txt) {
      ...
      // FIX: nid.szTip[0 .. txt.length] = txt[];
      tipLen = txt.length;
...
class NotifyIconControl : Control {
   override void createHandle() {
      ...
      hwnd = CreateWindowExA(wexstyle, CONTROL_CLASSNAME.ptr, "NotifyIcon", 0,
         0, 0, 0, 0, null, null, Application.getInstance(), null);
      if (!hwnd) {
         goto create_err;
      }
   }
   ...
   protected override void wndProc(ref Message msg) {
      if (msg.msg == WM_NOTIFYICON) {
         if (cast(UINT) msg.wParam in allNotifyIcons) {
            ...
            case WM_MOUSEMOVE:
               pt = Cursor.position;
               ni.mouseMove(ni, new MouseEventArgs(Control.mouseButtons(), 0, pt.x,
                  pt.y, 0));
               break;

            case WM_LBUTTONUP:
               pt = Cursor.position;
               ni.mouseUp(ni, new MouseEventArgs(MouseButtons.LEFT, 1, pt.x, pt.y,
                  0));
            ...
            case WM_RBUTTONUP:
               pt = Cursor.position;
               ni.mouseUp(ni, new MouseEventArgs(MouseButtons.RIGHT, 1, pt.x, pt.y,
                  0));
            ...
            case WM_LBUTTONDOWN:
               pt = Cursor.position;
               ni.mouseDown(ni, new MouseEventArgs(MouseButtons.LEFT, 0, pt.x, pt.y,
                  0));
               break;

            case WM_RBUTTONDOWN:
               pt = Cursor.position;
               ni.mouseDown(ni, new MouseEventArgs(MouseButtons.RIGHT, 0, pt.x, pt.y,
                  0));
               break;
