...
static:
   ...
   @property RegistryKey classesRoot() {
      if (!_classesRoot) {
   ...
   @property RegistryKey currentConfig() {
      if (!_currentConfig) {
   ...
   @property RegistryKey currentUser() {
      if (!_currentUser) {
   ...
   @property RegistryKey localMachine() {
      if (!_localMachine) {
   ...
   @property RegistryKey performanceData() {
      if (!_performanceData) {
...
abstract class RegistryValue {
   @property DWORD valueType();
   override Dstring toString();
   /+ package +/
   protected LONG save(HKEY hkey, Dstring name); // package
   package final @property RegistryValue _reg() {
      return this;
   }
}
...
class RegistryValueSz : RegistryValue {
   ...
   override @property DWORD valueType() {
      return REG_SZ;
   }
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      auto valuez = unsafeStringz(value);
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_SZ,
         cast(BYTE*) valuez, value.length + 1);
   }
}
...
class RegistryValueMultiSz : RegistryValue {
   ...
   protected override LONG save(HKEY hkey, Dstring name) { // package
      ...
      foreach (Dstring s; value) {
         if (!s.length) {
            throw new DflRegistryException(
               "Empty strings are not allowed in multi_sz registry values");
         }
      ...
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_MULTI_SZ,
         cast(BYTE*) multi, multi.length);
   }
}
...
class RegistryValueExpandSz : RegistryValue {
   ...
   protected override LONG save(HKEY hkey, Dstring name) { // package
      auto valuez = unsafeStringz(value);
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_EXPAND_SZ,
         cast(BYTE*) valuez, value.length + 1);
   }
}
...
class RegistryValueDword : RegistryValue {
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_DWORD,
         cast(BYTE*)&value, DWORD.sizeof);
   }
}
...
class RegistryValueDwordBigEndian : RegistryValue {
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_DWORD_BIG_ENDIAN,
         cast(BYTE*)&value, DWORD.sizeof);
   }
}
...
class RegistryValueBinary : RegistryValue {
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_BINARY,
         cast(BYTE*) value, value.length);
   }
}
...
class RegistryValueLink : RegistryValue {
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_LINK,
         cast(BYTE*) value, value.length);
   }
}
...
class RegistryValueResourceList : RegistryValue {
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_RESOURCE_LIST,
         cast(BYTE*) value, value.length);
   }
}
...
class RegistryValueNone : RegistryValue {
   ...
   /+ package +/
   protected override LONG save(HKEY hkey, Dstring name) { // package
      return RegSetValueExA(hkey, unsafeStringz(name), 0, REG_NONE,
         cast(BYTE*) value, value.length);
   }
}
...
final:
   ...
   final @property int subKeyCount() {
      DWORD count;

      LONG rr = RegQueryInfoKeyA(hkey, null, null, null, &count, null, null,
   ...
   final @property int valueCount() {
      DWORD count;

      LONG rr = RegQueryInfoKeyA(hkey, null, null, null, null, null, null,
   ...
   final RegistryKey createSubKey(Dstring name) {
      ...
      LONG rr = RegCreateKeyExA(hkey, unsafeStringz(name), 0, null, 0,
         KEY_ALL_ACCESS, null, &newHkey, &cdisp);
      if (ERROR_SUCCESS != rr) {
      ...
      if (ERROR_SUCCESS == RegOpenKeyExA(shkey, namez, 0, KEY_ALL_ACCESS, &openHkey)) {
         ...
         if (ERROR_SUCCESS == querycode) {
            ...
            del_me:
               ...
               next_subkey:
                  len = skn.length;
                  LONG enumcode = RegEnumKeyExA(openHkey, 0, skn.ptr, &len,
                     null, null, null, null);
                  switch (enumcode) {
                  case ERROR_SUCCESS:
   ...
   final RegistryValue getValue(Dstring name, RegistryValue defaultValue) {
      ...
      len = 0;
      LONG querycode = RegQueryValueExA(hkey, unsafeStringz(name), null, &type, null,
         &len);
      switch (querycode) {
      case ERROR_SUCCESS:
         // Good.
      ...
      querycode = RegQueryValueExA(hkey, unsafeStringz(name), null, &type, data.ptr,
         &len);
      if (ERROR_SUCCESS != querycode) {
         goto errquerycode;
      }
