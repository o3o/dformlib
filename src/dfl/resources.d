...
version (DFL_NO_RESOURCES) {
   ...
   class Resources {
      ...
      body {
         ...
         +/
         HICON hi;
         hi = cast(HICON) LoadImageA(hinst, cast(LPCSTR) cast(WORD) id,
            IMAGE_ICON, 0, 0, defaultSize ? (LR_DEFAULTSIZE | LR_SHARED) : 0);
         if (!hi) {
            return null;
         }
      ...
      final Icon getIcon(Dstring name, int width, int height) {
         ...
         // _noload("icon");
         HICON hi;
         hi = cast(HICON) dfl.internal.utf.loadImage(hinst, name, IMAGE_ICON, width,
            height, 0);
         if (!hi) {
            return null;
         }
      ...
      final Bitmap getBitmap(Dstring name) {
         HBITMAP h;
         h = cast(HBITMAP) loadImage(hinst, name, IMAGE_BITMAP, 0, 0, 0);
         if (!h) {
            return null;
         }
         return new Bitmap(h, true); // Owned.
      ...
      final Cursor getCursor(Dstring name) {
         HCURSOR h;
         h = cast(HCURSOR) loadImage(hinst, name, IMAGE_CURSOR, 0, 0, 0);
         if (!h) {
            return null;
         }
         return new Cursor(h, true); // Owned.
      ...
      body {
         ...
         wchar* ws = cast(wchar*) _getData(cast(LPCWSTR) RT_STRING,
            cast(LPCWSTR) cast(WORD)(id / 16 + 1)).ptr;
         Dstring result;
         if (ws) {
            int i;
            for (i = 0; i < (id & 15); i++) {
