...
class SplitterEventArgs : EventArgs {
   ...
   final @property void splitX(int val) {
      _splitX = val;
   }

   final @property int splitX() {
      return _splitX;
   }

   final @property void splitY(int val) {
      _splitY = val;
   }

   final @property int splitY() {
      return _splitY;
   }
...
class Splitter : Control {
   ...
   /+
   override @property void anchor(AnchorStyles a) {
   ...
   @property void movingGrip(bool byes) {
      if (mgrip == byes) {
         return;
      }
   ...
   protected override void onPaint(PaintEventArgs ea) {
      ...
      if (mgrip) {
         ea.graphics.drawMoveGrip(displayRectangle, DockStyle.LEFT == dock
            || DockStyle.RIGHT == dock);
      }
   }
   ...
   final @property void minExtra(int min) {
      mextra = min;
   }


   final @property int minExtra() {
      return mextra;
   }
   +/
   ...
   final @property void splitPosition(int pos) {
      auto splat = getSplitControl(); // Splitted.
      if (splat) {
         // DMD 0.95: need 'this' to access member dock
         //switch(dock())
   ...
   final @property int splitPosition() {
      auto splat = getSplitControl(); // Splitted.
      if (splat) {
         // DMD 0.95: need 'this' to access member dock
         //switch(dock())
