...
class TabPage : Panel {
   ...
   /+
   final @property void toolTipText(Dstring ttt) {
      // TODO: ...
   }
   ...
   package final @property void realBounds(Rect r) {
   ...
   package final @property void realVisible(bool byes) {
   ...
   body {
      ...
      if (dfl.internal.utf.useUnicode) {
         ...
         foreach (int i, TabPage page; _pages) {
            ...
            tci.tciw.pszText = cast(typeof(tci.tciw.pszText)) dfl.internal.utf.toUnicodez(
               page.text); // <--
...
public:
   ...
   mixin ListWrapArray!(TabPage, _pages, _blankListCallback!(TabPage), _added,
      _blankListCallback!(TabPage), _removed, true, false, false, true); // CLEAR_EACH
}
...
class TabControlBase : ControlSuperClass {
   ...
   final @property void drawMode(TabDrawMode dm) {
      switch (dm) {
   ...
   final @property TabDrawMode drawMode() {
   ...
   protected override void prevWndProc(ref Message msg) {
      ...
      msg.result = dfl.internal.utf.callWindowProc(tabcontrolPrevWndProc,
...
class TabControl : TabControlBase {
   this() {
      tchildren = new TabPageCollection(this);
      _pad = Point(6, 3);
   }

   final @property void alignment(TabAlignment talign) {
      switch (talign) {
   ...
   final @property TabAlignment alignment() {
   ...
   final @property void appearance(TabAppearance tappear) {
      switch (tappear) {
   ...
   final @property TabAppearance appearance() {
   ...
   final @property void padding(Point pad) {
      if (created) {
   ...
   final @property Point padding() {
      return _pad;
   }

   final @property TabPageCollection tabPages() {
      return tchildren;
   }

   final @property void multiline(bool byes) {
      if (byes) {
   ...
   final @property bool multiline() {
      return (_style() & TCS_MULTILINE) != 0;
   }

   final @property int rowCount() {
   ...
   final @property int tabCount() {
      return tchildren._pages.length;
   }

   final @property void selectedIndex(int i) {
   ...
   final @property int selectedIndex() {
      if (!created || !tchildren._pages.length) {
         return -1;
      }
      Message m;
   ...
   final @property void selectedTab(TabPage page) {
      int i;
   ...
   final @property TabPage selectedTab() {
      int i;
      i = selectedIndex;
      if (-1 == i) {
         return null;
   ...
   final @property void showToolTips(bool byes) {
      if(byes) {
   ...
   final @property bool showToolTips() {
